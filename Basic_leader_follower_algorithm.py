...
def run_leader_follower_setup():
    ...
    print("\n")
    print(
        "Please insert the value of the the threshold theta and then press ENTER (make sure it is a float number - I.E. 2.0)")
    theta = float(input())
    print("\n")
    print(
    ...
    def run_with_normalisation(self):
        ...
        for iteration, i in enumerate(self.order_of_indexes):
            ...
            if np.linalg.norm(x - rhino_centroid) < self.theta:
                ...
                rhino_centroid = np.divide(rhino_centroid,
                                           np.linalg.norm(rhino_centroid))  # Normalise updated cluster center
