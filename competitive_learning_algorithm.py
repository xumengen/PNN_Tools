...
def run_competitive_learning_setup():
    ...
    print(
        "Please insert the initial centroids. Each centroid should be divided by a space and each coordinate within a centroid should be divided by a coma. \nBe careful not to enter spaces after the coma that separates the centroids' coordinates.\n")
    ...
    print(
        "Please insert the value of the Learning Rate and then press ENTER (make sure it is a float number - I.E. 0.1)")
    lr = float(input())
    print("\n")
    print(
        "Please insert the order to follow when selecting the samples within the algorithm. \nYou should provide a sequential list of the indexes of the samples to select separated by single spaces.")
    print("I.E. 1 2 1 5 3")
...
class CompetitiveLearning:
    ...
    def run_with_normalisation(self):
        ...
        for iteration, i in enumerate(self.order_of_samples):
            ...
            print(
                f"The selected centroid is c{j + 1} {rhino_centroid}, with a net inner product to {x} of {net_inner_products[j]}")
    ...
    def run_without_normalisation(self):
        for iteration, i in enumerate(self.order_of_samples):
            ...
            print(
                f"The selected centroid is c{j + 1} {rhino_centroid}, with a distance of {distances_to_centroids[j]} to {x}")
    ...
    def classify_samples(self):
        ...
        if self.mode == 'without norm':
            for j, sample in enumerate(self.dataset):
                ...
                print(
                    f"Sample x{j + 1} {sample} belongs to cluster c{closest_centroid_index + 1} {self.centroids[closest_centroid_index]}")
            print("\n")
    ...
    def classify_new_data(self):
        ...
        if self.mode == 'without norm':
            print(
                "Please insert the new sample to classify. Do not insert spaces and divide its coordinates with a coma")
            ...
            for i, centroid in enumerate(self.centroids):
                ...
                if not closest_centroid_index or minimum_distance > dist:
                    ...
                    closest_centroid_index = i
            print(
                f"The new sample {new_sample} belongs to cluster c{closest_centroid_index + 1} {self.centroids[closest_centroid_index]}")
        elif self.mode == 'with norm':
            print("This feature is not available in mode \"with normalisation\". No examples were given.")
            print("\n")
